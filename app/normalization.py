...
_DOMAIN_ALIASES: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # В каталоге отрезные и лепестковые расходники называются дисками,
    # в разговоре — кругами; шлифовальный круг при этом остаётся кругом.
    (re.compile(r"\bкруг\s+отрезн\w*|\bотрезн\w*\s+круг\w*\b"), "диск отрезной"),
    (
        re.compile(r"\bкруг\s+лепестков\w*|\bлепестков\w*\s+круг\w*\b"),
        "диск лепестковый",
    ),
...
_QUERY_NOISE_RE = re.compile(
    ...
    r"посовет\w*|подсказ\w*|подскаж\w*|скаж\w*|сказ\w*|"
    r"сейчас|сегодня|завтра|срочно|прямо|когда|"
    # Условия сделки: словами о доставке и оплате товар не описывают, но
    # рядом с названием товара они встречаются постоянно.
    r"спасибо|благодарю|самовывоз|достав\w*|гаранти\w*|кредит|рассрочк\w*|оплат\w*|"
...
# Формы слов упаковки: `\w*` не покрывает беглую гласную в родительном
# падеже множественного числа (`5 пачек`, `10 упаковок`).
_PACK_WORDS = r"пач(?:к\w*|ек)|упаковк\w*|упаковок|уп\.?"

_LEXICAL_ORDER_NOISE_RE = re.compile(
    rf"\b(?:{_PACK_WORDS}|шт(?:\.|ук\w*)?|штук\w*|"
...
# Минус учитывается только вплотную к числу: `-50 мм` — это бессмысленный
# размер, а `саморезы 3.5x45 - 200 шт` — обычное тире вместо запятой.
_NEGATIVE_NUMBER_RE = re.compile(
    r"(?:(?<![a-zа-я0-9])-\d|[xх×*]\s*-\s*\d)", re.IGNORECASE
)
_CODE_RE = re.compile(
...
_ORDER_AMOUNT_PATTERNS = (
    ...
    re.compile(
        ...
        rf"пар(?:а|ы|у|ой|ами|ах)?|листа|листов|листами|{_PACK_WORDS})\b"
    ),
    re.compile(
        r"\b(?:шт(?:\.|ук\w*)?|штук\w*|метр\w*|килограмм\w*|кг|"
        rf"пар(?:а|ы|у|ой|ами|ах)?|лист(?:а|ов|ами)|{_PACK_WORDS})\s+"
        r"(?P<value>\d+(?:\.\d+)?)\b"
    ),
...
_UNIT_HINTS: tuple[tuple[re.Pattern[str], str], ...] = (
    (re.compile(rf"\b(?:{_PACK_WORDS})\b"), "уп"),
